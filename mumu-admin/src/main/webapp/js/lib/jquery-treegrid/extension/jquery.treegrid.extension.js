(function($) {
	...
	$.fn.bootstrapTreeTable = function(options, param) {
		...
		// 缓存请求到的数据
		target.setCacheRootNodes = function(data){
			target.cacheRootNodes = data;
		}
		var j = 0;
		// 递归获取子节点并且设置子节点
		...
		target.load = function(parms){
			...
			var thead = $('<thead class="treegrid-thead" style="background-color: #f2f2f2"></thead>');
			thead.append(thr);
			target.append(thead);
			// 构造表体
